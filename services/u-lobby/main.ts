// The u-lobby: the Ubisoft lobby the game itself connects to.
//
//   node services/u-lobby/main.ts [--host 127.0.0.1] [--bind 0.0.0.0] [--http 8080] [--ghosts] [--quiet-bot]
//
// This was `tools/net-server.ts`, the one process that was everything, and then a while
// the "gateway" — a name that said where it sat instead of what it was. What left it is
...
//
// A game that cannot reach this network at all comes through the door instead: its mod
// holds the u-lobby sockets on its own loopback and carries them here over one WebSocket,
// upgraded at `/u-lobby` on the same port as everything else (`services/u-lobby/tunnel.ts`).
//
// The log goes to logs/u-lobby-latest.log AND, unchanged, to logs/latest.log — see
...
import { probePeerAddress, probeRoomFields, roomClosed, roomComputers, roomEndpoints, roomFacts, roomMap } from './lobby.ts';
 ...
 * the first thing it says and not in the port it said it on (`services/u-lobby/classify.ts`).
 *
...
const log = openLog('u-lobby', { alsoPlainLatest: true });
 ...
 *
 * The door hangs on this server too: an upgrade to `/u-lobby` is a tunnelled game handing
 * its u-lobby sockets over (`services/u-lobby/tunnel.ts`), and `/health` is for whoever
 * watches the deployment. Everything else said in HTTP is the game's curl — absolute-form
 * through a proxy or a bare path — and the ini is the one answer it is owed.
 */
const iniServer = createHttpServer((req: IncomingMessage, res: ServerResponse) => {
  if (req.url === '/health') {
    const body = JSON.stringify({ ok: true, carrying: carry.clients() });
...
/**
 * The door: a game that reaches us through a tunnel carries its u-lobby sockets in over
 * one WebSocket, and every stream and datagram inside lands back on this same port as an
 * ordinary loopback connection — the classifier reads it like anything the game dialled.
 */
const carry = carryULobby({ server: iniServer, target: { host: '127.0.0.1', port: httpPort }, log });
...
// Every key the player types is accepted; see services/u-lobby/cdkey-service.ts for why
...
// makes the client join an IRC channel. See services/u-lobby/irc.ts.
...
/** Who this process is, so its own echo is recognisable. Two u-lobbies will not collide. */
const U_LOBBY_ID = `u-lobby-${process.pid}`;
...
const core = new CoreClient({ url: settings.coreUrl, service: 'u-lobby', log });

core.onChat = (message, sender) => {
  if (sender === U_LOBBY_ID) return; // already drawn, by the fan-out below
 ...
 * (`services/u-lobby/classify.ts`). Nothing can be built at connect time, then: the session
 ...
 * remember, so every one of them is classified on its own (`services/u-lobby/classify.ts`).
...
function openUdpWindow(port: number): void {
  ...
  udp.on('message', (data: Buffer, from) => {
    ...
    // in services/u-lobby/nat-service.ts for the race that makes that necessary.
